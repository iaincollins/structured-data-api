   ...
   if (_models != null)
    ...
    glob(schemaDir+"/**/*.json", {}, function (er, files) {
      files.forEach(function(file) {
        /**
         * The model name is taken from the file name for the schema file.
         *
         * If the schema is in the root of the schema dir, the collectionName
         * will be a pluralized form of the model name.
         * 
         * If the model is in a sub directory, the collectionName will be a 
         * pluralized form of the parent directory path.
         *
         * e.g.
         *
         * ../schemas/Person.js:                           collectionName = 'persons'
         * ../schemas/Event.js:                            collectionName = 'events'
        ...
        var collectionName = camelCase(pluralize.plural( path.dirname(path.relative(__dirname, file)).replace(/^\.\.\/schemas(\/?)/, '').replace(/\//g, '-') || modelName ));
          
        collectionName = 'entities';
        
        _loadJsonSchemaAsModel(jsonSchema, modelName, collectionName)
...
function _loadJsonSchemaAsModel(jsonSchema, modelName, collectionName) {
  var promise = new Promise(function(resolve, reject) {
    var mongooseSchema = createMongooseSchema({}, jsonSchema);
    // Add _type field to all models (used to store model name)
    mongooseSchema._type = { type: String };
    
    /*
    Sypport for JSON Schema types beyond currently supported primitives
    ...
    */
    ...
    schema.pre('save', function(next) {
      ...
      // @TODO Custom hooks
      // hook(this, next(), modelName, collectionName, jsonSchema);
      next();
    });
